...
export default function MessagingSystem() {
  ...
  useEffect(() => {
    if (session?.user) {
      ...
      socketInstance.on('connect', () => {
        socketInstance.emit('join', { userId: session.user.id, userName: session.user.name })
      })
      ...
      socketInstance.on('previousMessages', (msgs: Message[]) => {
        setMessages(msgs)
      })
  ...
  const sendMessage = () => {
    if (newMessage.trim() && socket && session?.user) {
      const message = {
        text: newMessage,
